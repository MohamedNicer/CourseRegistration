...
export default cds.service.impl(async function(this: any) {
    ...
    const { Instructors: InstructorsDB, Courses: CoursesDB } = cds.entities('com.sap.capire.courseregistration');
    ...
    this.before('UPDATE', Enrollments, async (req: any) => {
        ...
        const { Enrollments: EnrollmentsDB } = cds.entities('com.sap.capire.courseregistration');
    ...
    this.after('UPDATE', Enrollments, async (data: any, req: any) => {
        ...
        if (courseId && oldStatus !== newStatus && (oldStatus === 'ENROLLED' || newStatus === 'ENROLLED')) {
            ...
            const { Enrollments: EnrollmentsDB, Courses } = cds.entities('com.sap.capire.courseregistration');
    ...
    this.before('READ', Enrollments, async (req: any) => {
        ...
        if (userEmail) {
            ...
            const result = await cds.run(`SELECT ID, email FROM com_sap_capire_courseregistration_Instructors`);
            const instructor = result.find((i: any) => i.email.toLowerCase() === userEmail.toLowerCase());
            ...
            if (instructor) {
                ...
                const courses = await cds.run(`SELECT ID FROM com_sap_capire_courseregistration_Courses WHERE instructor_ID = ${instructor.ID}`);
    ...
    this.before('READ', Instructors, async (req: any) => {
        ...
        if (userEmail) {
            ...
            const result = await cds.run(`SELECT ID, email, firstName, lastName FROM com_sap_capire_courseregistration_Instructors`);
            console.log(`[InstructorService] Instructors - Total instructors in DB (via cds.run): ${result.length}`);
            if (result.length > 0) {
                console.log(`[InstructorService] Instructors - All emails:`, result.map((i: any) => `${i.ID}:${i.email}`));
            }
            ...
            if (instructor) {
                ...
                console.warn(`[InstructorService] Instructors - Looking for: ${userEmail}, Available:`, result.map((i: any) => i.email));
    ...
    this.after('READ', Courses, async (results: any, req: any) => {
        ...
        const { Departments, Enrollments: EnrollmentsDB } = cds.entities('com.sap.capire.courseregistration');
        ...
        for (const course of courses) {
            ...
            if (course.ID) {
                ...
                const { Courses: CoursesDB } = cds.entities('com.sap.capire.courseregistration');
                await cds.update(CoursesDB).set({ enrolled: enrolledCount.length }).where({ ID: course.ID });
            }
        }
